...
public class SpeechToTextService
{
    ...
    public async Task<string> ListenAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[SpeechToText] ListenAsync error: {ex.Message}");
            return string.Empty;
        }
        finally
        {
    ...
    private async Task<string> RecognizeInternalAsync(CancellationToken ct)
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine($"[SpeechToText] Windows error: {ex.Message}");
            return string.Empty;
        }
    }
#elif ANDROID
    private async Task<string> RecognizeInternalAsync(CancellationToken ct)
    {
        ...
        await MainThread.InvokeOnMainThreadAsync(async () =>
        {
            try
            {
                ...
                intent.PutExtra(Android.Speech.RecognizerIntent.ExtraPrompt, "Please speak");
            ...
            {
                System.Diagnostics.Debug.WriteLine($"[SpeechToText] Android error: {ex.Message}");
                tcs.TrySetResult(string.Empty);
            }
        });
